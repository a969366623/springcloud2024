...
public class PayAlibabaController {
    ...
    public R getPayByOrderNo(@PathVariable("orderNo") String orderNo)
    {
        ...
        return R.success("查询返回值："+payDTO);
    }
    public R handlerBlockHandler(@PathVariable("orderNo") String orderNo, BlockException exception)
    {
        return R.fail(ReturnCodeEnum.RC500.getCode(),"getPayByOrderNo服务不可用，" +
                "触发sentinel流控配置规则"+"\t"+"o(╥﹏╥)o");
    }
    /*
    fallback服务降级方法纳入到Feign接口统一处理，全局一个
    public R myFallBack(@PathVariable("orderNo") String orderNo,Throwable throwable)
    {
        return R.fail(ReturnCodeEnum.RC500.getCode(),"异常情况："+throwable.getMessage());
    }
    */
}
